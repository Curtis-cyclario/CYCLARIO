...
import type { Waveform, UserPattern, AudioSourceMetric, AudioProfile, Interconnects } from '../types';
import { SavePatternModal } from './SavePatternModal';
import { Tooltip } from './Tooltip';
import { InterconnectMatrix } from './InterconnectMatrix';
...
interface ControlsProps {
  ...
  // Interconnect additions
  interconnects: Interconnects;
  onInterconnectToggle: (type: 'rows' | 'cols', index: number) => void;
}
...
export const Controls: React.FC<ControlsProps> = (props) => {
  const { 
    ...
    onOpenGenerator, interconnects, onInterconnectToggle
  } = props;
      ...
      <div className="component-panel w-full p-4 rounded-lg flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-2">
                    <div className="flex justify-between items-center">
                        <label htmlFor="border-toggle" className="text-xs text-cyan-400/70 tracking-widest uppercase">Cell Borders</label>
                        <Tooltip text={showBorders ? "Hide cell borders" : "Show cell borders"}>
                            ...
                            >
                                <span className={`inline-block w-4 h-4 transform bg-white rounded-full transition-transform duration-200 ease-in-out ${showBorders ? 'translate-x-6' : 'translate-x-1'}`} />
                            </button>
                        </Tooltip>
                    </div>
                </div>
                <Tooltip text="Clears the lattice, setting all cells to inactive"><Button onClick={onClear} disabled={isRunning} className="text-gray-300 bg-gray-800/50 border-gray-600/50 hover:bg-gray-700/50 hover:border-gray-500 focus:ring-gray-500 w-full mt-2">CLEAR LATTICE</Button></Tooltip>
            </div>

            {/* Optical Interconnects Management */}
            <div className="flex flex-col h-full">
                <InterconnectMatrix 
                    channels={interconnects}
                    onToggle={onInterconnectToggle}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="flex flex-col gap-2">
           ...
           <div className="flex flex-col gap-2">
            ...
            <Tooltip text="Adjusts the master volume">
