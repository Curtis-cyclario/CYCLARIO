...
export const useAudio = () => {
    ...
    const [audioSource, setAudioSource] = useState<AudioSourceMetric>('delta_swastika');
    ...
    const playFeedbackSound = useCallback((metrics: MetricsData) => {
        ...
        switch(audioSource) {
            case 'invariance':
                // Invariance is typically 0.0 to 1.0. We map lower invariance (errors) to higher sound?
                // Or map 1.0 to silence? Let's map changes to sound.
                // 1.0 is perfect. Sound should trigger on imperfection.
                normalizedValue = 1.0 - rawValue; 
                break;
            case 'reversibility':
                 // 1.0 is perfect. Sound on entropy.
                normalizedValue = 1.0 - rawValue;
                break;
            case 'delta_swastika':
        ...
        if (normalizedValue <= 0.01) return; // Silence threshold

        const soundVolume = 0.1 + normalizedValue * 0.4;

        switch(audioProfile) {
            case 'synth_blip': {
                const baseFreq = 200;
                const freqRange = 1000;
            ...
            case 'thermal_noise': {
                ...
                noiseGain.gain.linearRampToValueAtTime(soundVolume * 0.5, now + 0.03); 
